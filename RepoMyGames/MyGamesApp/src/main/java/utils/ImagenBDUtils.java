...
public class ImagenBDUtils {
  ...
  // Lista de consolas retro
  private static final String[] consolasRetro = { "NES", "SNES", "Nintendo 64", "GameCube", "Game Boy",
      "Game Boy Color", "Game Boy Advance", "Nintendo DS", "Nintendo 3DS", "SEGA Genesis", "SEGA Saturn",
      "SEGA Dreamcast", "Atari 2600", "Atari 5200", "Atari 7800", "Neo Geo" };

  // Método para asignar las imágenes a los elementos de la interfaz
  public static void asignarImagenes(VBox vbox, List<JuegoBD> juegos) {
    if (juegos.isEmpty()) {
      return; // Si la lista está vacía, no hacer nada
    }

    JuegoBD juego = juegos.get(0); // Tomar el primer juego de la lista
  ...
  // Método auxiliar para convertir un array de bytes a una imagen
  public static Image convertirImagen(byte[] imagenBytes) {
    if (imagenBytes == null) {
      return null;
    }
