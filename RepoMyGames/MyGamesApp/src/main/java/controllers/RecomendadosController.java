...
public class RecomendadosController {
	...
	public void cargarJuegosBiblioteca() {
		String generoRaw = MetodosSQL.obtenerGeneroAleatorio(HomeController.getUsuario().getGenerosPreferidos())
				.toLowerCase();

		String genero = generoRaw.replace(" ", "-");
		System.out.println(genero);
		if (genero.contains("rpg")) {
			genero = "5";
		}
		...
		for (int i = 0; i < total; i++) {
			if (i < chunkSize) {
				lista1.add(juegosRecomendados.get(i));
			} else if (i < chunkSize * 2) {
				lista2.add(juegosRecomendados.get(i));
			} else if (i < chunkSize * 3) {
				lista3.add(juegosRecomendados.get(i));
			} else {
				lista4.add(juegosRecomendados.get(i));
			}
		}
	...
	public static void setUltimoTextoBusqueda(String ultimoTextoBusqueda) {
		RecomendadosController.ultimoTextoBusqueda = ultimoTextoBusqueda;
	}

	private final static String PANEL_USER = "/views/CambiarInfoPersonal.fxml";
	private final static String PANEL_BIBLIIOTECA = "/views/Biblioteca.fxml";
	...
	void imgLupaPressed(MouseEvent event) throws IOException {
		...
		VentanaUtil.abrirVentana(PANEL_HOME_BUSQUEDA, "Búsqueda", STYLES, controller -> {
			((RecomendadosController) controller).setJuegosBuscados(juegosCargar);
		}, event);
	...
	void imgCargarJuegosPressed(MouseEvent event) throws IOException {
		...
		List<JuegoHome> juegosCargar = ExtractorAPI.buscarJuegosPorGenero(genero, indice);
		System.out.println("JUEGOS CARGADOS:" + juegosCargar.size());
		...
		VentanaUtil.abrirVentana(PANEL_RECOMENDADOS, "Búsqueda", STYLES, controller -> {
			((RecomendadosController) controller).setJuegosBuscados(juegosCargar);
		}, event);
	...
	void imgCargarJuegoAtrasPressed(MouseEvent event) throws IOException {
		...
		VentanaUtil.abrirVentana(PANEL_RECOMENDADOS, "Recomendados", STYLES, controller -> {
			((RecomendadosController) controller).setJuegosBuscados(juegosCargar);
		}, event);
	}
