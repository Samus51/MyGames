...
public class LoginController {
  ...
  void btnLoginPressed(MouseEvent event) {
    ...
    if (cone == null) {
      mostrarError("No se pudo conectar a la base de datos. Verifique su conexión.");
      return;
    }
    ...
    try (PreparedStatement st = cone.prepareStatement(SQL_USUARIO)) {
      ...
      if (rs.next()) {
        ...
        if (BCrypt.checkpw(password, storedHash)) {
          ...
          Usuario usuario = new Usuario(rs.getInt("id_usuario"), rs.getString("nombre"), rs.getString("email"),
              storedHash, Arrays.asList(rs.getString("generos").split(",")));
