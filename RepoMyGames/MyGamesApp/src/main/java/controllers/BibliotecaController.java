...
public class BibliotecaController {
	...
	public void cargarJuegosBiblioteca() {
		...
		List<JuegoHome> juegosBiblioteca = MetodosSQL.obtenerJuegosBiblioteca(conn,
				HomeController.getUsuario().getIdUsuario());

		// Dividir la lista en 4 partes intentando que cada parte tenga 3 juegos
		int total = juegosBiblioteca.size();
		int chunkSize = 3; // Cada lista tendrá 3 juegos como máximo
		...
		for (int i = 0; i < total; i++) {
			if (i < chunkSize) {
				lista1.add(juegosBiblioteca.get(i));
			} else if (i < chunkSize * 2) {
				lista2.add(juegosBiblioteca.get(i));
			} else if (i < chunkSize * 3) {
				lista3.add(juegosBiblioteca.get(i));
			} else {
				lista4.add(juegosBiblioteca.get(i));
			}
		}
	...
	public static void setUltimoTextoBusqueda(String ultimoTextoBusqueda) {
		BibliotecaController.ultimoTextoBusqueda = ultimoTextoBusqueda;
	}
	...
	void imgLupaPressed(MouseEvent event) throws IOException {
		...
		VentanaUtil.abrirVentana(PANEL_HOME_BUSQUEDA, "Búsqueda", STYLES, controller -> {
			((BibliotecaController) controller).setJuegosBuscados(juegosCargar);
		}, event);
